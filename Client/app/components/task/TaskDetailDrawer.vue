...
import { generateId, STATUS_CONFIG, PRIORITY_CONFIG, isDueOverdue, isDueSoon } from '~/utils'
...
const { state, dispatch, activeTask, getUserById, getSubtasks, getTaskById } = useAppStore()
const toast = useToast()

const newSubtask = ref('')

const task = activeTask
const open = computed(() => activeTask.value !== null)

const creator = computed(() => task.value ? getUserById(task.value.createdBy) : null)
const assignees = computed(() =>
  task.value?.assignees.map(id => getUserById(id)).filter(Boolean) ?? []
)
const subtasks = computed(() =>
  task.value ? getSubtasks(task.value.id) : []
)
...
const dependencies = computed(() =>
  task.value?.dependencies.map(id => getTaskById(id)).filter(Boolean) ?? []
)
...
function changeStatus(newStatus: TaskStatus) {
  ...
  const message: Message = {
    ...
    type: 'status-change',
    content: `marked ${task.value.title} as ${STATUS_CONFIG[newStatus].label.toLowerCase()}`,
    senderId: state.value.currentUser.id,
    taskId: task.value.id,
    timestamp: new Date().toISOString(),
    metadata: { from: task.value.status, to: newStatus }
  }
...
function changePriority(newPriority: TaskPriority) {
  ...
  const message: Message = {
    ...
    threadId: task.value.threadId,
    type: 'task-updated',
    content: `Changed priority of ${task.value.title} to ${PRIORITY_CONFIG[newPriority].label}`,
    senderId: state.value.currentUser.id,
    taskId: task.value.id,
    timestamp: new Date().toISOString()
  }
...
function toggleSubtask(subtaskId: string) {
  ...
  dispatch({
    type: 'UPDATE_TASK',
    task: { ...subtask, status: newStatus, updatedAt: new Date().toISOString() }
  })
...
function addSubtask() {
  if (!newSubtask.value.trim() || !task.value) {
    toast.add({
  ...
  const message: Message = {
    ...
    threadId: task.value.threadId,
    type: 'task-created',
    content: `Created subtask: ${newSubtask.value.trim()}`,
    senderId: state.value.currentUser.id,
    taskId: subtaskObj.id,
    timestamp: now
  }
  dispatch({ type: 'ADD_TASK', task: subtaskObj, message })
  toast.add({
...
function handleDelete() {
  ...
  dispatch({ type: 'DELETE_TASK', taskId: task.value.id })
