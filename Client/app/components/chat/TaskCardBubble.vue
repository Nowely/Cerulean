...
import { formatTime, STATUS_CONFIG, isDueOverdue, isDueSoon } from '~/utils'
...
const { state, dispatch, getUserById, getTaskById, getSubtasks } = useAppStore()

const sender = computed(() => getUserById(props.message.senderId))
const task = computed(() => props.message.taskId ? getTaskById(props.message.taskId) : null)
const isOwn = computed(() => props.message.senderId === state.value.currentUser.id)
...
function handleClick() {
  if (task.value) {
    dispatch({ type: 'SET_ACTIVE_TASK', taskId: task.value.id })
  }
}
</script>
