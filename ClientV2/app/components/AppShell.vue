...
import TaskTemplatePicker from '~/components/task/TaskTemplatePicker.vue'

const { state, dispatch, activeThread } = useAppStore()
const isMobile = useIsMobile()

const sidebarOpen = computed(() => state.value.sidebarOpen)
const mobileSidebarOpen = computed(() => isMobile.value && sidebarOpen.value)

watch(isMobile, (mobile) => {
  if (!mobile && state.value.sidebarOpen) {
...
<template>
  <div class="flex h-dvh w-full overflow-hidden">
    ...
    <USlideover
      ...
      side="left"
      :ui="{ content: 'w-80' }"
      @update:open="(o) => dispatch({ type: 'SET_SIDEBAR', open: o })"
    >
      <template #content>
        <Sidebar />
      </template>
