...
<template>
  <header v-if="activeThread" class="flex h-14 shrink-0 items-center justify-between border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 px-3">
    <div class="flex items-center gap-3">
      ...
      <div class="flex flex-col">
        <div class="flex items-center gap-2">
          <h2 class="text-sm font-semibold">{{ activeThread.name }}</h2>
          <UBadge
    ...
    <div class="flex items-center gap-1">
      <AvatarStack :users="members" :max-visible="3" size="sm" />
      <button
      ...
      >
        <UIcon name="i-lucide-bell" class="h-4.5 w-4.5" />
        <span
